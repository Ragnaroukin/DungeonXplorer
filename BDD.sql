...
CREATE TABLE Links (
    link_choice INT,
    aventure_id INT, -- source
    chapter_id INT,  -- source
    link_aventure_id INT, -- destination
    link_chapter_id INT, -- destination
    description TEXT
...
CREATE TABLE Joueur (
    ...
    joueur_pseudo VARCHAR(60) NOT NULL,
    joueur_mdp VARCHAR(60) NOT NULL,
    joueur_admin BOOLEAN DEFAULT 0, -- le joueur est-il administrateur
    joueur_image VARCHAR(255) -- chemin de la photo de profil
);
...
ALTER TABLE Links ADD CONSTRAINT pk_Links PRIMARY KEY(link_choice,aventure_id,chapter_id);
...
ALTER TABLE Chapter ADD CONSTRAINT fk_Chapter FOREIGN KEY (aventure_id) REFERENCES Aventure(aventure_id);
...
ALTER TABLE Hero ADD CONSTRAINT fk5_Hero FOREIGN KEY (joueur_id) REFERENCES Joueur(joueur_id);
